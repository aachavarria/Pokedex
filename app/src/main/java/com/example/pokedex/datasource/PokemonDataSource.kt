...
class PokemonDataSource(private val apiService: APIService, val keyword: String) : RxPagingSource<Int, Pokemon>() {
    override fun loadSingle(params: LoadParams<Int>): Single<LoadResult<Int, Pokemon>> {
        ...
        paramObject.put("query", "{species:pokemon_v2_pokemonspecies_aggregate(where:{name: {_ilike:\"%${keyword}%\"}}){list:aggregate{count}}list:pokemon_v2_pokemonspecies(offset:${offset},limit:${limit},order_by:{id:asc},where:{name: {_ilike:\"%${keyword}%\"}}){id name details:pokemon_v2_pokemons(distinct_on:pokemon_species_id){types:pokemon_v2_pokemontypes{type:pokemon_v2_type {name}}}}}")
